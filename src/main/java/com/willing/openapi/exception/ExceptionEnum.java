...
public enum ExceptionEnum {
    ...
    WILLING_ENCRYPTION_AND_DECRYPTION_FAILED(1014, "卫瓴加解密失败", true),
    WILLING_ENCRYPTION_AND_DECRYPTION_DATA_EXCEPTION(1015, "卫瓴加解密数据异常", true),
    ...
    public static Set<Integer> alterCode(){
        return Arrays.stream(ExceptionEnum.values()).filter(ExceptionEnum::isAlert)
                .map(ExceptionEnum::getCode).collect(toSet());
    }
}
